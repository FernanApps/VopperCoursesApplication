...
actual class XVideoPlayer(
    ...
    private val eventAdapter = object : MediaPlayerEventAdapter() {
        override fun buffering(mediaPlayer: MediaPlayer?, newCache: Float) {
            if (newCache == 100.0f) {
                _status.value = if (mediaPlayer?.status()?.isPlaying == true) XPlayerStatus.Playing else XPlayerStatus.Paused
            } else {
                _status.value = XPlayerStatus.Buffering
            }
        }
        ...
        override fun error(mediaPlayer: MediaPlayer?) {
            ...
            _status.value = XPlayerStatus.Error(
                Error(
